...
app.get("/lists", 
  ...
  catchError(async(req, res, next) => {
    ...
    let todoLists = await store.sortedTodoLists(username);
  ...
  catchError(async(req, res) => {
    ...
    const rerenderCreateList = () => {
      ...
      if (!errors.isEmpty()) {
        ...
        let created = await store.createList(todoListTitle);
        if (!created) {
          req.flash("error", "The list title must be unique.");
          rerenderCreateList();
        } else {
...
requiresAuthentication, 
  ...
  let todoListId = req.params.todoListId;
  let store = res.locals.store;
  let allDone = store.markAllDone(+todoListId);
  ...
  catchError(async(req, res, next) => {
    ...
    if (!todoList) throw new Error("Not found.");
      ...
      if (!errors.isEmpty()) {
        ...
        let created = store.addTodo(+todoListId, todoTitle);
